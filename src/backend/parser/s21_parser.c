...
ObjData parse_obj(const char *file_path, int *parse_flag) {
    ...
    if (token != NULL) {
      if (strcmp(token, "v") == 0) {
        ...
        while (next != NULL) {
          ...
          objData.vertexCount++;
          objData.vertices = realloc(objData.vertices,
                                     (objData.vertexCount + 1) * sizeof(float));
          next = strtok(NULL, " ");
        }
        ...
        while (next != NULL) {
          ...
          objData.faceCount++;
          objData.faces = realloc(
              objData.faces, (objData.faceCount + 1) * sizeof(unsigned int));
  ...
  // printf("vertex array size %d %d\n", sizeof(objData.vertices),
  // sizeof(Vertex)); printf("faces array size %d %d\n", sizeof(objData.faces),
  // sizeof(unsigned int));
